...
func add(path string, m *map[string]func(float64, float64) float64) {
	...
	handle := C.dlopen(pathCString, C.RTLD_LAZY)
	...
	keyPtr := C.dlsym(handle, keyPtrCString)
	...
	valuePtr := C.dlsym(handle, valuePtrCString)
	...
	key := C.Operator(keyPtr)
	defer C.free(unsafe.Pointer(key))

	(*m)[C.GoString(key)] = func(f1 float64, f2 float64) float64 {
